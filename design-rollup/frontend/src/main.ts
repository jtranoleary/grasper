...
function createGeometryFromSimulation(): THREE.LatheGeometry {
    ...
    // Simulation returns Top to Bottom (+Y to -Y).
    // LatheGeometry expects Bottom to Top (-Y to +Y) for correct Normals (CCW).
    // So we iterate backwards.
    for (let i = rawPoints.length - 2; i >= 0; i -= 2) {
        points.push(new THREE.Vector2(rawPoints[i], rawPoints[i + 1]));
    }

    // DEBUG: Check for monotonicity
    let yMonotonic = true;
    let vMonotonic = true;
    const vs = glassSim.get_vs().reverse(); // We verified we need to reverse vs to match points order (bottom-up)

    // Points are now Bottom-to-Top (increasing Y expected)
    for (let i = 0; i < points.length - 1; i++) {
        if (points[i + 1].y < points[i].y) {
            console.error(`Y Monotonicity VIOLATION at i=${i}: ${points[i].y} -> ${points[i + 1].y}`);
            yMonotonic = false;
        }
    }

    // Vs are also reversed, so they should be 0.0 -> 1.0 (monotonic increasing)
    for (let i = 0; i < vs.length - 1; i++) {
        // Allow slight float error or equal values
        if (vs[i + 1] < vs[i] - 1e-6) {
            console.error(`V Monotonicity VIOLATION at i=${i}: ${vs[i]} -> ${vs[i + 1]}`);
            vMonotonic = false;
        }
    }

    if (!yMonotonic || !vMonotonic) {
        console.warn("Simulated data has folds or scrambles! Texture artifacts expected.");
    }

    const geometry = new THREE.LatheGeometry(points, 64);

    // We also need to reverse the VS to match the reversed points
    // const vs = glassSim.get_vs().reverse(); // Already fetched above
    applyCylindricalUVs(geometry, vs);

    return geometry;
}

function applyCylindricalUVs(geometry: THREE.BufferGeometry, profileVs: Float32Array) {
    geometry.computeBoundingBox();

    // Debug logs
    if (profileVs.length > 0) {
        console.log(`Applying UVs. ProfileVs points: ${profileVs.length}. Range: [${profileVs[0].toFixed(3)}, ${profileVs[profileVs.length - 1].toFixed(3)}]`);
    } else {
        console.warn("Applying UVs but profileVs is empty!");
    }

    // LatheGeometry creates (segments + 1) vertices per profile point
    const segments = 64;
    const verticesPerRow = segments + 1;

    console.log(`Geometry vertices: ${geometry.attributes.position.count}. Expected rows: ${geometry.attributes.position.count / verticesPerRow}`);
    ...
    for (let i = 0; i < posAttribute.count; i++) {
        ...
        // V comes from the simulation state (pinned to material)

        // V comes from the simulation state (pinned to material)
        // LatheGeometry creates vertices row-by-row.
        // LatheGeometry generates vertices internally by iterating:
        // Outer Loop: Segments (around the circle)
        // Inner Loop: Points (up the profile)
        // Therefore, the buffer is ordered by "Cylinder Strips" (Meridians).
        // Adjacent vertices in the buffer are adjacent vertically on the glass.

        // We need to map the 'v' coordinate from our profileVs array.
        // Since the inner loop iterates through the profile points, the index corresponds directly to our profile array.
        const pointIndex = i % profileVs.length;
        const v = profileVs[pointIndex];

        // Preserve the existing U coordinate generated by LatheGeometry
        // This handles the seam (0.0 vs 1.0) correctly, whereas atan2 is discontinuous.
        const u = uvAttribute.getX(i);

        uvAttribute.setXY(i, u, v);
    }

    // specific debug: Check UV ranges
    ...
    console.log(`UV Stats: U[${minU.toFixed(2)}, ${maxU.toFixed(2)}], V[${minV.toFixed(2)}, ${maxV.toFixed(2)}]`);
